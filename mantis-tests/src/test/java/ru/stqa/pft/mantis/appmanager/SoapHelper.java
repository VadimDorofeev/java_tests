...
public class SoapHelper {
    ...
    public Issue getIssueById(int issueId) throws MalformedURLException, ServiceException, RemoteException {
        ...
        return new Issue().withId(issue.getId().intValue()).withProject(new Project().withId(issue.getProject().getId().intValue()).withName(issue.getProject().getName())).withDescription(issue.getDescription())
                .withSummary(issue.getSummary()).withResolution(issue.getResolution().getName());
    }
}
